...
def main(argv):
    ...
    h_range = np.arange(0.005,.15,.005)
    
    data_dt=1
    delay=3

    ctraj_path = '/bucket/StephensU/antonio/BehaviorModel/Zebrafish/ctrajs_562_clusters_delay_{}/'.format(delay)
    ...
    kf = np.random.randint(0,len(ctraj_fish))
    X = ctraj_fish[kf]
    ...
    for kh,h in enumerate(h_range):
        ...
        print(h,kf,h_D1,h_D2,flush=True)
    
   

    print('Saving results',flush=True)
    f = h5py.File('/flash/StephensU/antonio/BehaviorModel/Zebrafish/optimize_h_delay_{}/delta_h_{}.h5'.format(delay,idx),'w')
