...
def optimal_partition(phi2,inv_measure,P,return_rho = True):
    ...
    for kc,c in enumerate(c_range):
        labels = np.zeros(len(phi2),dtype=int)
        labels[phi2<=c] = 1
    ...
    if len(peaks)==0:
        ...
        c_opt = c_range[idx]
        kmeans_labels = np.zeros(len(phi2),dtype=int)
        kmeans_labels[phi2<c_opt] = 1

        if return_rho:
            return c_range,rho_sets,c_opt,kmeans_labels
        else:
...
def subdivide_state_optimal(state_to_split,phi2,kmeans_labels,inv_measure,P,indices,plot):
